...
export async function getCheckoutURL(variantId?: number, embed = false) {
  ...
  const checkout = await createCheckout(env.LEMONSQUEEZY_STORE_ID, variantId, {
    ...
    productOptions: {
      redirectUrl: getAbsoluteUrl(siteUrls.organization.plansAndBilling),
  ...
  const subscriptionsCountByMonth = months.map((month) => {
    ...
    const count =
      subscriptions.data?.data.filter(
        (subscription) =>
          format(new Date(subscription.attributes.created_at), 'MMM-yyy') ===
          monthStr
      )?.length ?? 0;
...
export async function getRevenueCount() {
  ...
  const totalRevenue =
    orders.data?.data.reduce((acc, order) => acc + order.attributes.total, 0) ??
    0;
  ...
  const revenueCountByMonth = months.map((month) => {
    ...
    const revenueCount =
      orders.data?.data
        .filter(
          (order) =>
            format(new Date(order.attributes.created_at), 'MMM-yyy') ===
            monthStr
        )

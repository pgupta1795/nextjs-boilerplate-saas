...
const content = {
  logoAlt: 'Somvarsha logo',
  docsSrOnly: 'Docs',
  githubSrOnly: 'Somvarsha github',
      ...
      <footer className='container grid grid-cols-1 gap-8 border border-border bg-background p-8 sm:grid-cols-2 sm:rounded-lg'>
        <div className='grid place-content-between gap-2'>
          <div className='grid gap-2'>
            <Link href={siteUrls.dashboard.home} className={cn('z-10')}>
              <Icons.logo
                className='text-xl'
                iconProps={{
              ...
              />
              <span className='sr-only'>{content.logoAlt}</span>
            </Link>
          ...
          <div className='flex items-center gap-2'>
            ...
            >
              ...
              <span className='sr-only'>{content.docsSrOnly}</span>
            </Link>
            <Link
        ...
        <div className='grid grid-cols-2 gap-4 sm:grid-cols-3'>
          <div className='flex flex-col gap-2'>
            <h3 className='text-sm font-semibold'>{content.resourcesHeader}</h3>
            {navigation.map((item) => (
              <FooterLink
                key={item.label}
                href={item.href}
            ...
            <FooterLink href={siteUrls.github} label='Github' external />
          </div>
          <div className='flex flex-col gap-2'>
            <h3 className='text-sm font-semibold'>
              {content.usefulLinksHeader}
            </h3>
            {navigation.map((item) => (
              <FooterLink
                key={item.label}
          ...
          <div className='flex flex-col gap-2'>
            <h3 className='text-sm font-semibold'>{content.moreHeader}</h3>
            {navigation.map((item) => (
              <FooterLink
                key={item.label}
                href={item.href}
